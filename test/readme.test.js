...
/**
 * Images rot exactly like prose: a file gets renamed and the README shows a
 * broken image to everyone on npm, where nobody who could fix it is looking.
 * They are generated by `npm run screenshots`, so they can at least be checked.
 */
test('every screenshot the README references exists', () => {
  const referenced = [...readme.matchAll(/docs\/screenshots\/([\w-]+\.png)/g)].map((m) => m[1]);
  assert.ok(referenced.length > 0, 'the README should show the UI it describes');
  const present = readdirSync(join(root, 'docs', 'screenshots'));
  const missing = [...new Set(referenced)].filter((f) => !present.includes(f));
  assert.deepEqual(missing, [], 'referenced but not committed: ' + missing.join(', '));
});

test('every screenshot has both a light and a dark variant', () => {
  // The <picture> blocks swap on theme; a missing half shows the wrong one.
  const files = readdirSync(join(root, 'docs', 'screenshots')).filter((f) => f.endsWith('.png'));
  const bases = new Set(files.map((f) => f.replace(/-(light|dark)\.png$/, '')));
  for (const base of bases) {
    assert.ok(files.includes(base + '-light.png'), base + ' has no light variant');
    assert.ok(files.includes(base + '-dark.png'), base + ' has no dark variant');
  }
});

test('no committed screenshot is unused', () => {
  const files = readdirSync(join(root, 'docs', 'screenshots')).filter((f) => f.endsWith('.png'));
  const orphans = files.filter((f) => !readme.includes(f));
  assert.deepEqual(orphans, [], 'committed but never shown: ' + orphans.join(', '));
});

test('screenshots are referenced by absolute URL, so npm can render them', () => {
  // npm serves the README from the tarball, where a relative path resolves to
  // nothing. GitHub is forgiving about this and npm is not.
  const relative = [...readme.matchAll(/(?:src|srcset)="([^"]*docs\/screenshots[^"]*)"/g)]
    .map((m) => m[1])
    .filter((url) => !url.startsWith('https://'));
  assert.deepEqual(relative, [], 'relative image paths break on npm: ' + relative.join(', '));
});

test('screenshots stay out of the published package', () => {
  // `files` is a whitelist, so docs/ is excluded — but only while it stays one.
  assert.ok(Array.isArray(pkg.files), 'package.json should whitelist what ships');
  assert.ok(!pkg.files.includes('docs'), 'docs/ would add megabytes to every install');
});
